...
\title{
  \includegraphics[natheight=913,natwidth=712, height=0.3\textheight]{images/qub-logo.png}
  \\Ethical Considerations for the $21^{st}$ Century in Bioinformatics and Medical Practice}
...
\begin{abstract}
Information Technology along with genetics is playing a critical role in
the $21^{st}$ century in Bioinformatics. The field of Bioinformatics requires
Information Technology to analyse and distribute vast data sets. This article
discusses some of the elements of Bioinformatics that pose ethical issues
and measures that are taken to protect medical data.

Biobanks are an important large data source in Bioinformatics, one such example
is was set up by deCODE in Iceland using blood samples and anonymised medical
data belonging to Icelandic citizens. Criticism of biobanks have been made regarding
the protection of personal data as protected by the
\emph{European Directive on the protection of Personal Data}. Explicit consent of
patients should be sought in the form of clear and rigorous consent forms vetted by
the relevant authorities.

Electronic Medical Records (EMR) are popular among primary care surgeries in the United
Kingdom, however, uptake of this is less popular in the United States. Benefits include
efficiency as well as accuracy when using these systems in conjunction with E-prescribing.
However, care should be taken to secure these records which may contain sensitive data
about patients. Where ethical considerations are not adhered to, problems can arise such
as the reading of Gordon Brown and Alex Salmond's private medical data by unauthorised parties.

Bioimaging will be used increasingly in pathology labs in the coming years. It is important
to recognise that images are data in and of themselves and as such, privacy considerations
must also be adhered to. In research care must be taken to ensure that images are manipulated
in a justifiable way so as not to undermine the integrity of the scientific findings that
depend on them.
\end{abstract}
  
\section{Introduction}
In 2006, Professor Herman Tavani wrote that both Information Technology and
genetics will play a critical role in $21^{st}$ century healthcare and research. As part of
